...
** Input/Output
*NOTE*: The notations for $i$ and $j$ are not consistent; look for superscript or subscript
*** Multiple Training points (superscript)
+ $(x^{(i)}, y^{(i)})$ is a single training pair
+ $x^{(i)}$ is the x-value of the $i$ th training pair
+ Multiple points would be horizontal
+ $m$ training points
$$x = \begin{bmatrix}
x^{(1)} & x^{(2)} & x^{(3)} & \cdots & x^{(m)} \end{bmatrix}$$
$$y = \begin{bmatrix}
y^{(1)} & y^{(2)} & y^{(3)} & \cdots & y^{(m)} \end{bmatrix}$$
*** Multiple features (subscript)
+ $x_j$ representr the $j$ th feature
+ $x_j^{(i)}$ is the $i$ th training point for the $j$ th feature
+ Multiple features are vertical
+ $n$ features
+ $x_0 = 1$
$$x = \begin{bmatrix}
...
*NOTE*: Very important to do feature scaling
*** Feature dependance
+ Given $x_1$(length) and $x_2$ (width)
+ Create new feature (area):
$$x_3 := x_1 \cdot x_2$$
*** Polynoial features
+ Creating closer fits
